# 群晖 + Sub-Store + OpenWrt 旧版简化部署参考

本文保留旧版顺序转换链路，供历史配置比较。D2 固定端口部署改用明确映射，先读 [D2 部署门禁与迁移说明](DEPLOY_D2.md)。不要按以下旧流程升级 D2 的固定端口消费者。
...
- `other` 不使用前三槽和日常质量重排：`rebuild` 全量检测后建立一次完整质量
  ...
  `rebuild` 才会重新排列 `other`。从旧镜像升级时会直接继承当前已发布的
  `other` 顺序，不会自动触发一次全量重建。
...
node-health 必须读取原始 `inventory`，不能读取已经按上一版状态重排的
`healthy`，否则会形成排序反馈循环。
...
- 输出节点数和完整输入一致，风险、不可达和未检测节点仍在但顺序靠后；
- URL 带 `noCache=true`。
