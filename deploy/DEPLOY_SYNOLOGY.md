...
机场订阅
  ...
  -> OpenWrt 下载 inventory + current.json -> 本地严格过滤 -> local-socks / AdsPower / TXT
```

扫描输入必须是 `inventory`，普通订阅客户端使用 `healthy`。OpenWrt 也下载 `inventory`，但由本地转换器按同一份 `current.json` 再做一次严格白名单过滤；这样 local-socks 不依赖 Script Operator 的缓存或临时故障。不要让 node-health 读取 `healthy`，否则会形成反馈循环，已经被过滤的节点无法重新恢复。
...
如果状态文件不存在或其中没有任何已记录的稳定槽位身份，服务会自动把计划任务或手动 `maintenance` 升级为 `rebuild`。已记录但暂时 unavailable/absent 的槽位仍算有效历史，不会触发自动重选。损坏且无法解析的状态文件不会被静默忽略，服务会停止发布并要求从备份恢复或由你明确清理后重建，避免意外丢弃仍有价值的槽位历史。
...
它会对全部节点快速检测，完整检测稳定槽位、新节点、出口 IP 变化节点、超过 48 小时未深检的节点及每区前 10 个候补。
...
| 单次超时、临时不可用或检测结果不确定 | 保留原 `node_key`，等待恢复 | 端口可能暂时不可用，但不改指向 |
| 节点暂时从 `inventory` 消失 | 保留原 `node_key`，不拿候补顶位 | 暂停生成该槽位监听；同一身份恢复后回到原槽位 |
| 明确触发质量红线 | 只用最高分合格候补替换这个槽位 | 仅该端口变更身份，其他四槽不动 |
...
- 至少连续 `2` 次完整检测通过，达到高置信度；
- 至少 `2` 个风险评分源返回有效数据，AI 可用性为明确通过；
- 默认 3 次出口采样至少成功 2 次，出口 IP 一致，实际地区与节点地区一致；
- 综合分至少比当前“可比较的最弱稳定节点”高 `12` 分；
- 同一地区本轮最多晋级 `1` 个节点；
- 该地区任一稳定槽位距离上次身份变化，至少已经冷却 `7` 天。

“可比较”只包含当前存在、可用且检测数据足以比较的稳定节点。临时 unavailable 的稳定节点和暂时不在 inventory 的 ghost 槽位不参与“最弱节点”比较，因此自动晋级不能借一次掉线或订阅短暂缺失清退它们。如果该地区尚在冷却期，或没有可比较的稳定节点，本轮不执行性能晋级。

除了当前分差，服务还会核对上一轮可比得分是否同样至少领先 `12` 分，以过滤单日偶然峰值。冷却期只限制“为了更高质量而晋级”，不阻止明确质量红线的安全替换。每次晋级、红线替换或 `rebuild` 后，都会重新计算该地区的 7 天晋级冷却期；同一地区仍然遵守每轮最多晋级一个节点。

稳定槽位之后的动态节点，也就是逻辑上的第 4 名及以后，每天按照置信度、质量分和延迟重新排序。动态节点确认不可用或触发红线时可直接从 `healthy` 输出移除。
...
- 临时不可用或暂时缺失的稳定槽位标记为“保留等待恢复”，并显示固定端口当前可能不可用；不会伪装成已替换。
...
健康脚本采用两层保护：成功读取有效 `current.json` 时，稳定槽位按固定身份顺序输出，只有第 4 名以后的节点才按 `ranked` 健康排序。动态节点确认不可用、触发红线或不再位于本轮白名单时会删除；稳定槽位临时不可用不会被候补改写，暂时从 `inventory` 消失时则自然没有可输出的代理对象，但槽位身份仍保留在健康状态中。
...
如果 `002` 只是临时不可用或暂时从订阅消失，`62801` 不会改绑候补：有代理对象但无法出网时端口会不可用，订阅中没有该对象时则暂时不生成监听。只有 `002` 明确触发质量红线、被满足全部门槛的候补受控晋级替换，或运行了全量 `rebuild`，才允许更换其身份。发生单槽替换时只更新 `62801`，其他稳定端口不移动；没有合格候补时该端口继续留空，动态候选仍从 `62805` 开始。
...
curl --socks5-hostname 127.0.0.1:62804 --max-time 20 https://api.ipify.org
```
...
- 稳定节点单次不可用或暂时从 inventory 缺失：保留槽位身份，不自动拿候补顶位。
