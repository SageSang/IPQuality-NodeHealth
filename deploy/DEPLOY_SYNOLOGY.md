...
如果 `002` 只是临时不可用或暂时从订阅消失，`62801` 不会改绑候补：有代理对象但无法出网时端口会不可用，订阅中没有该对象时则暂时不生成监听。只有 `002` 明确触发质量红线、被满足全部门槛的候补受控晋级替换，或运行了全量 `rebuild`，才允许更换其身份。发生单槽替换时只更新 `62801`，其他稳定端口不移动；没有合格候补时该端口继续留空，动态候选仍从 `62803` 开始。
