...
def build_local_socks_exports(
    ...
    Regional files remain the canonical fixed-port views. ``all.txt`` is the
    same lines concatenated in configured region order for one-shot imports.
    """
    ...
    for item in assessments:
        ...
        entries.setdefault(item.node.region, []).append(
            (
                position[0],
                f"socks5://{advertise_host}:{position[1]}{{{name}}}",
            )
        )
    rendered = {
