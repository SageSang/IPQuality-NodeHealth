...
仓库现已包含可由群晖 Container Manager 部署的节点健康服务，支持两种模式：日常 `maintenance` 保持每个地区 1-3 的固定身份，每天深检稳定槽位并轮转抽检动态池的四分之一；连接参数变化的逻辑同一节点在当轮强制深检，临时不可用前两轮等待恢复、连续第 3 轮失败才换槽并降到地区队尾，明确质量红线则立即替换对应槽位。连续两个不同自然日可信深检且当前与前一日均领先至少 10 分的高置信候补可受控晋级。首次部署、状态丢失或手动触发 `rebuild` 时，全量检测并重新竞选每区最优 1-3。服务还提供独立的订阅审计 API，可对任意获准的 Sub-Store ClashMeta 链接执行全节点检测并归档详细报告，而不修改正式排名。
